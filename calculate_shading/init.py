...
class ProcessDSM():
    ...
    def __init__(self, DSM_PATH, HOUSE_SHP_PATH=None, crs='EPSG:27700'):
        ...
        extent = QgsVectorLayer(self.DSM_path, '', 'ogr' ).extent()
        self.HOUSE_SHP_PATH = self.clip_polygon(HOUSE_SHP_PATH, extent)
    ...
    def calculate_slope(self, layer):
        ...
        Input:
        layer(str): Path to DSM tile (.tif)

        Output:
        (str): Path to slope raster layer (.tif)
        """
    ...
    def calculate_aspect(self, layer):
        ...
        Output:
        (str): Path to aspect raster layer (.tif)
        """
    ...
    def reclass(self, layer, table):
        ...
        Input:
        layer(str): Path to raster layer (.tif)
        foldername(str): 'slope' or 'aspect'
        table(list): [MIN, MAX, CLASS, MIN, MAX, CLASS, ...]

        Output:
        (str): Path to reclassed raster layer (.tif)
        """
        print(f"Reclassing...")
        start = time.time()
    ...
    def sieve(self, layer, threshold=10):
        ...
        Output:
        (str): Path to sieved raster layer (.tif)
        """
        print(f"Sieving...")
        start = time.time()
    ...
    def clip_raster_by_mask(self, layer):
        ...
        Output:
        (str): Path to clipped raster (.tif)
        """
    ...
    def clip_polygon(self, layer, extent):
        ...
        Input:
        layer(str): Path to raster layer (.tif)
        extent(str): 4-point coordinates of extent to clip to
        """
    ...
    def polygonize(self, layer, field):
        ...
        Inputs:
        layer(str): Path to clipped raster layer (.tif)
    ...
    def buffer(self, layer_path, distance, output_path = 'TEMPORARY_OUTPUT'):
        ...
        Input:
        layer_path(str): Path to layer for buffer (.gpkg)
    ...
    def zonal_statistics(self, layer, mask, prefix = ''):
        ...
        Input:
        layer(str): Path to polygon shapefile (.gpkg)
    ...
    def merge_vector_layers(self, layerA, layerB, fields):
        ...
        Output:
        (str): Path to merged vector layer (.gpkg)
        """
    ...
    def filter_polygons(self, layer):
        ...
        filter_params = {
            'INPUT':layer,
            'EXPRESSION':' "AREA" > 16 AND 0 < "slope_mean" <= 60 AND 67.5 <= "aspect_mean" <=292.5 AND "shading_mean" >= 0.5',
    ...
    def roof_segmentation(self, DSM):
        ...
        params = {
            ...
            'aspect' : {
                ...
                'table': ['0','45','1','45','135','2','135','225','3','225','315','4','315','360','1'],
                'threshold': 2
            }
        }
...
class ProcessOSMap(ProcessDSM):
    def __init__(self, HOUSE_SHP_PATH, crs='EPSG:27700'):
        ...
        self.extent = QgsVectorLayer(self.HOUSE_SHP_PATH, '', 'ogr' ).extent()
        print(self.extent)
...
def main():
    ...
    # DSM_PATH = "C:\\Users\\lilia\\Documents\\GitHub\\WMCA\\scripts\\DSM\\sp0585_DSM_1M.tif"
    # DTM_path = "C:\\Users\\lilia\\Documents\\GitHub\\WMCA\\LIDAR\\LIDAR-DTM-1m-2020-SP08ne\\SP08ne_DTM_1m.tif"
    ...
    # height_path = program.calculate_height(DSM_path, DTM_path)
