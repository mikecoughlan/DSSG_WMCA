...
def get_models():
	models = dict()
	n_var = [1e-5, 1e-7, 1e-9, 1e-11, 1e-13]
	for n in n_var:
		models[str(n)] = GaussianNB(priors=None, var_smoothing=n)
	return models
...
def NaiveBayes_tuning(X_train, X_test, y_train, y_test, predicting):
	...
	plt.savefig('plots/model_tuning/{0}/naivebayes_var_smoothing_tuning.png'.format(predicting))



#Naive Bayes Model
def NaiveBayes_model(X_train, X_test, y_train, y_test, predicting, version):
	'''
		INPUTS:
			X_train (np.array)
			X_test (np.array)
		...
		RETURNS:
			y_pred (np.array): predicted probabilistic results from the naive bayes model
			'''

	NB = GaussianNB(priors=None,var_smoothing=1e-13)
	NB.fit(X_train, y_train)
	y_pred = NB.predict_proba(X_test)
	# row_sums = np.sum(y_pred, axis=1) # normalization 
	# row_sums = np.reshape(row_sums, (len(row_sums),1))
	# copy_array = row_sums
	# row_sums = np.pad(row_sums,((0,0),(0,y_test.shape[1]-1)), mode="edge")
	# y_pred = np.divide(y_pred, row_sums)

	with open('models/{0}/NaiveBayesClassifier_version_{1}.h5'.format(predicting, str(version)), 'wb') as f:
		pickle.dump(NB, f)
...
def main(target, stage):
	...
	if stage == 'eval':
		...
		y_pred = NaiveBayes_model(X_train, X_test, y_train, y_test, predicting, version)
		if predicting == 'epc':
			metrics = mf.calculating_epc_metrics(y_test, [y_pred], ['NB_version_{0}'.format(version)])
			print(metrics)

		if predicting == 'mainheat':
			metrics = mf.calculating_heat_metrics(y_test, [y_pred], ['NB_version_{0}'.format(version)])
			print(metrics)
		y_pred = pd.DataFrame(y_pred)
		y_pred.to_csv('outputs/{0}/NaiveBayes_version_{1}.csv'.format(predicting, str(version)), index=False)
