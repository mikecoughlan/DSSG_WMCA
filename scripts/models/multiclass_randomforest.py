...
CONFIG = {
		'random_int': 123,
		# 'input_features': ['postcode_epc', 'constituency', 'msoa_code', 'lsoa_code',
...
def loading_testing_data(test_df=None, file_path=None):
	...
	if test_df.empty:
		...
		if file_path==None:
			test_df = pd.read_csv(DATA_PATH+'testing_file_name.csv')	# loading the testing dataframe


	X_test = test_df[CONFIG['input_features']]					# dropping everything not in the 'input_features'
	print("GET RID OF THE NEXT LINE AFTER THIS GOES ON ARIDHIA!!!!!")
...
def fitting_and_predicting(X_train, X_test, y_train, predicting, to_fit, file_name):
	...
	if to_fit:
		...
		# saving the model after it has been fit
		with open('models/{0}/{1}.h5'.format(predicting, file_name), 'wb') as f:
			pickle.dump(model, f)

	if to_fit==False:
		with open('models/{0}/{1}.h5'.format(predicting, file_name), 'rb') as f:
			model = pickle.load(f)
...
def fitting_and_predicting_regression(X_train, X_test, y_train, predicting, to_fit, file_name):
	''' 
		Function that does the fitting of the defined model on the 
		training data, saves the fit model, and then makes a prediction 
		on the testing data.
...
def main(train_df, test_df, target, predicting, saved_file_name, to_fit=True, file_path=None):
	...
	y_pred = fitting_and_predicting(X_train, X_test, y_train, predicting=predicting, to_fit=to_fit, file_name='multiclass_randomforest')	 # model training and predicting

	if predicting == 'epc':

		y_pred_eff = fitting_and_predicting_regression(X_train, X_test, y_train_eff, predicting=predicting, to_fit=to_fit, file_name='regression_randomforest')
	...
	output_df.to_csv('outputs/{0}/{1}.csv'.format(predicting, saved_file_name), index=False)			# saving the file.
