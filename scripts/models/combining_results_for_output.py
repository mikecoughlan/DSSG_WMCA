...
CONFIG = {
		'random_int': 123,
		'features_to_keep': ['uprn', 'postcode', 'lsoa_code', 'RelHMax', 'calculatedAreaValue',
							'total_consumption', 'mean_counsumption', 'median_consumption',
							'prop_households_fuel_poor', 'LATITUDE', 'LONGITUDE', 'mainheat-description',
							 'current-energy-rating', 'current-energy-efficiency', 'confidence', 
							 'confidence_within_one_rating', 'additional_load', 'additional_peak_load', 'predicted']}
...
def calculating_peak_ratio():
	...
	'''Calculating the peak ratio using the UK total energy demand. Will be used
		...
		heat pump for its heat source from a non-eectric heat source.
...
def calculating_additional_load(df, EPC_df, COP=1):
	...
	for area in areas: 		# looping through the definied areas
		...
		conditions = conditions + [(df['mainheat-description'] == 0) & (df['total-floor-area'] == area) & (df['current-energy-rating'] == rating) for rating in ratings]
...
def main(full_dataset=None, epc_df=None):
	...
	if full_dataset.empty:
		full_dataset = pd.read_csv('outputs/total_output.csv') 			# loading the full dataset
	...
	full_dataset = full_dataset[CONFIG['features_to_keep']]

	full_dataset.to_csv('outputs/full_dataset_outputs.csv', index=False)		# saving the dataset
...
if __name__ == '__main__':
	...
	full_dataset = main()      # calling the main function
