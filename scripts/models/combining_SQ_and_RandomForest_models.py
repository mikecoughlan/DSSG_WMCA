...
def comparing_results(df):
	...
	ratings = [df['current-energy-rating'], df['RF_current-energy-rating'], df['RF_current-energy-rating'], df['RF_current-energy-rating'], df['SQ_current-energy-rating'], df['RF_current-energy-rating']]
	confidence = [1, df['RF_confidence'], df['RF_confidence'], df['RF_confidence'], df['SQ_confidence'], df['RF_confidence']]
	within_one_conf = [np.nan, df['confidence_within_one_rating'], df['confidence_within_one_rating'], df['confidence_within_one_rating'], np.nan, df['confidence_within_one_rating']] 		# cannot provide a within on confidence level for SQ predictions
...
def main(df):
	...
	# df = pd.concat([sim, no_sim], axis=0)

	df.reset_index(inplace=True, drop=True)

	df.to_csv('outputs/combined_epc_ratings.csv', index=False)
