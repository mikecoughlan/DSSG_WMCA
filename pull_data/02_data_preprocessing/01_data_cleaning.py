...
class DataCleaning():
  def __init__(self, df, counties, DATA_PATH, PLOT_PATH):
    self.df = df
    self.pred = 'current-energy-efficiency'
    self.DATA_PATH = DATA_PATH
    self.PLOT_PATH = PLOT_PATH
    self.counties = counties 
  
  # def filter_geography(self):
  #   """ OPTIONAL Filter coordinates for those in region of interest """
  ...
  def clean_dependent_variable(self):
    ...
    self.df = self.df[self.df[self.pred] <=100]
    print(f"Min: {self.df[self.pred].min()}")
    print(f"Max: {self.df[self.pred].max()}")

    plt.figure(figsize=(20,5))
    plt.savefig(self.PLOT_PATH+'cleaned_epc_rating_distribution.png')
    ...
    for i, band in enumerate(bands):
      ...
      plt.subplot(241+i)
      bins = len(epc_bands[self.pred].unique())
      epc_bands[self.pred].hist(bins=bins)
  ...
  def missing_values_column(self):
    ...
    self.df = self.df.drop([
              ...
              'constituency-label', 'lmk-key', 'mainheat-env-eff', 'mainheatc-energy-eff',
              'walls-energy-eff','roof-env-eff','mainheatc-env-eff','walls-env-eff',
              'hot-water-energy-eff','windows-energy-eff',
  ...
  def missing_values_row(self):
    ...
    missing_plt.save_fig(self.PLOT_PATH+'missing_row_distribution.png')
  ...
  def clip_numeric(self):
    ...
    fig.tight_layout()
    fig.fig_save(self.PLOT_PATH+'numerical_var_boxplot.png')
    ...
    fig.tight_layout()
    fig.fig_save(self.PLOT_PATH+'clipped_numerical_var_boxplot.png')
